...
const PROPOSE_THRESHOLD_KCAL = 25;      // kcal move below this alone won't propose
const PROPOSE_THRESHOLD_PROTEIN_G = 5;  // ...but a protein move this large will
const PROPOSE_THRESHOLD_FAT_G = 5;      // ...or a fat move this large (fat is the swing macro)
...
export function recalcTargets({ profile, readings, now = new Date() }) {
  ...
  // Propose when ANY macro has moved past its noise floor — not kcal alone. kcal can sit
  // still while protein/fat need to move (e.g. the 1.6->2.0 protein rebase, or any week
  // where the deficit holds kcal flat but weight shifts protein). A kcal-only gate hides
  // those. Carbs are pinned at 20, so only kcal/protein/fat are checked.
  const kcalMove = old.target_kcal == null ? Infinity : Math.abs(next.target_kcal - old.target_kcal);
  const proteinMove = old.target_protein_g == null ? Infinity : Math.abs(next.target_protein_g - old.target_protein_g);
  const fatMove = old.target_fat_g == null ? Infinity : Math.abs(next.target_fat_g - old.target_fat_g);
  const thresholdMet =
    kcalMove >= PROPOSE_THRESHOLD_KCAL ||
    proteinMove >= PROPOSE_THRESHOLD_PROTEIN_G ||
    fatMove >= PROPOSE_THRESHOLD_FAT_G;
