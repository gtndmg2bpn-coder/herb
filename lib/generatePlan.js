...
function generatePlan(constraints, inventory, recipes, opts) {
  ...
  const eligible = seededShuffle(recipes, rng);
  const chosenBatchDays = batchDays.slice(0, nBatches);
  ...
  for (const bday of chosenBatchDays) {
    ...
    const totalPortions = Math.max(BATCH_SIZE, guestPortions);
  ...
  // A batch's fresh portions land within [batch_day, batch_day + FRESH_SHELF_DAYS].
  // Fill the batch day itself first (esp. guest days), then forward within range.
  const fillFromBatches = (slot) => {
    ...
    for (const b of batches) {
      ...
      if (gap < 0 || gap > FRESH_SHELF_DAYS) continue;
  ...
  if (emptyCount > 0) {
    rationale.push(
      `${emptyCount} slot${emptyCount > 1 ? 's' : ''} left empty — not enough stock or batch capacity to fill the week without breaking the variety cap.`
    );
  }
  ...
  const cleanBatches = batches.map(({ _freshLeft, ...b }) => b);
