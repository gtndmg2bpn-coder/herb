...
function generatePlan(constraints, inventory, recipes, opts) {
  ...
  const perDay = daysPlanned.map((d) => {
    ...
    return {
      ...
      kcal_in_band: targetingOn
        ...
        : null,
      protein_met: targetingOn ? led.protein_g >= targetProteinG : null,
    };
  });
  ...
  if (!targetingOn) {
    ...
    if (macroSummary.days_under_protein > 0) {
      rationale.push(
        `${macroSummary.days_under_protein} day${macroSummary.days_under_protein > 1 ? 's fall' : ' falls'} short of the ${targetProteinG} g protein floor.`
      );
    }
