...
function generatePlan(constraints, inventory, recipes, opts) {
  ...
  //
  // THREE GATES, all added 28 Aug after the first live week (Karum: "we've got
  // to have a way of working this without adding olive oil for the sake of it").
  // The first version had none of them and put 45 g — three tablespoons, its own
  // per-slot cap — onto a Thursday that was actually missing two meals.
  //
  //   1. THE WEEK, NOT THE DAY. Calories are a WEEKLY budget by design, so a
  //      low day inside a week that is already on target needs nothing at all;
  //      it is balanced by a high one. The first version looked only at the day,
  //      so on a week averaging 2035 against 2000 it still poured oil into the
  //      dips — making the week worse in the name of fixing a day. Nothing is
  //      added unless the WEEK is short, and never more than the week is short by.
  //
  //   2. COMPLETE DAYS ONLY. A day with an unfilled slot is not short of fat,
  //      it is short of a MEAL, and answering that with oil makes a broken day
  //      look merely disappointing. Those days are skipped and reported as what
  //      they are.
  //
  //   3. NEVER MORE THAN A SENSIBLE DOSE. Capped per slot by the diet, and if
  //      what is left after the cap is still a real gap, that gap is reported
  //      rather than drowned.
  ...
  let fatDaysClosed = 0;
  let fatDaysSkippedIncomplete = 0;
  let fatSkippedWeekOnTarget = false;
  if (targetingOn && fatLever && fatKcalPerG > 0) {
    const dates = Object.keys(dayLedger).sort();
    const weekTargetKcal = targetKcal * dates.length;
    const weekKcalBefore = dates.reduce((sum, d) => sum + dayLedger[d].kcal, 0);
    // GATE 1 — is the WEEK short? This is the budget the lever serves.
    let weekBudgetKcal = weekTargetKcal - weekKcalBefore;
    if (weekBudgetKcal <= 0) {
      fatSkippedWeekOnTarget = true;
    } else {
      for (const date of dates) {
        if (weekBudgetKcal <= 0) break;
        const led = dayLedger[date];
        if (led.filled === 0) continue;
        // GATE 2 — a day missing a meal is not a day missing fat.
        if (led.filled < (dayPlanCount[date] || 0)) { fatDaysSkippedIncomplete += 1; continue; }
        ...
        // Never more than this day needs, this week can absorb, or the dose allows.
        const allowedKcal = Math.min(targetKcal - led.kcal, weekBudgetKcal);
        const grams = Math.min(
          Math.round(allowedKcal / fatKcalPerG),
          daySlots.length * fatMaxPerSlot
        );
        ...
        for (const s of daySlots) {
          ...
          fatAddedTotalG += add;
          weekBudgetKcal -= add * fatKcalPerG;
        }
  ...
  const macroSummary = {
    ...
    fat_days_skipped_incomplete: fatDaysSkippedIncomplete,
    fat_skipped_week_on_target: fatSkippedWeekOnTarget,
  };
  ...
  if (!targetingOn) {
    ...
    if (fatSkippedWeekOnTarget) {
      // The single most important thing this feature does NOT do. Calories are a
      // weekly budget, so a dip balanced by a peak needs nothing at all, and
      // topping it up would push the week further over the target it is serving.
      rationale.push(
        `No ${what} was added — the week is already averaging at or above ${targetKcal} kcal, so a lighter day is balanced by a heavier one rather than topped up.`
      );
    }
    if (fatDaysSkippedIncomplete > 0) {
      rationale.push(
        `${fatDaysSkippedIncomplete} day${fatDaysSkippedIncomplete > 1 ? 's were' : ' was'} left short rather than topped up — ${fatDaysSkippedIncomplete > 1 ? 'they have' : 'it has'} unfilled slots, and a day missing a meal needs a meal, not ${what}.`
      );
    }
    if (targetingOn && !fatLever && macroSummary.days_kcal_out_of_band > 0) {
