...
function generatePlan(constraints, inventory, recipes, opts) {
  ...
  // "Off meat" defaults to dropping the four land meats and keeping fish + veg.
  // Override per-week by passing appetite.meat_types.
  const MEAT = new Set(appetite.meat_types || ['beef', 'pork', 'lamb', 'chicken']);
  const cuisineTerm = (appetite.cuisine || '').trim().toLowerCase();
  const matchesCuisine = (r) => !cuisineTerm ||
    [r.tag, r.section, r.name, r.blurb].some((v) => typeof v === 'string' && v.toLowerCase().includes(cuisineTerm));
  ...
  const blockReason = (r) => {
    ...
    if (!matchesCuisine(r)) return 'cuisine';
    return null;
  };
  const blockedCounts = {};
  ...
  const batchPool = allowedRecipes.filter((r) => isFreezable(r) && servesLD(r));
  const freshPool = allowedRecipes.filter((r) => r.freezes === false && servesLD(r));
  ...
  const pullOne = (pool, slot) => {
    const idx = pool.findIndex((p) => {
      ...
      return r && isAllowed(p.recipe_id) && canServe(r, slot.meal) && canTake(p.recipe_id, slot);
    });
  ...
  for (const lightMeal of ALL_MEALS.filter((m) => LIGHT_MEALS.has(m))) {
    ...
    const pickLight = (avoidProtein, slot) => {
      ...
      const min = Math.min(...free.map((r) => bCount[r.id] || 0));
      const leastUsed = free.filter((r) => (bCount[r.id] || 0) === min);
