...
// Builds the week's trips (main + top-up) from planned meals minus pantry.
export async function generateShoppingList({ weekStart = null, mainTripDate = null, topupTripDate = null }) {
  const supabase = getBrowserClient();
  return supabase.rpc('generate_shopping_list', {
    p_week_start: weekStart ?? todayIso(),
    p_main_trip_date: mainTripDate ?? todayIso(),
    p_topup_trip_date: topupTripDate,
  });
}
