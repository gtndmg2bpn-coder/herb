...
// Bins stock (ingredient or cooked portion): draws it down and records the
// waste cost — ingredients at their per-unit price, cooked portions at the
// per-portion recipe cost (same maths as eating).
export async function binStock({
  itemKind, quantity, location,
  ingredientId = null, recipeId = null, label = null, wastedDate = null,
}) {
  const supabase = getBrowserClient();
  return supabase.rpc('bin_stock', {
    p_item_kind: itemKind,
    p_quantity: quantity,
    p_location: location,
    p_ingredient_id: ingredientId,
    p_recipe_id: recipeId,
    p_label: label,
    p_wasted_date: wastedDate ?? todayIso(),
  });
}
...
// Writes confirmed daily targets to the caller's profile. This is the ONLY
// post-onboarding path that changes targets, and it only ever runs behind a user
// tap on the adaptive-targets proposal card. Values come straight from
// recalcTargets().new (already rounded integers). The RPC is security-definer and
// scoped to auth.uid() internally, so it can only ever write the caller's own row,
// and it touches ONLY the four target columns (+ updated_at).
export async function commitTargets({ kcal, proteinG, carbsG, fatG }) {
  const supabase = getBrowserClient();
  return supabase.rpc('commit_targets', {
    p_target_kcal: kcal,
    p_target_protein_g: proteinG,
    p_target_carbs_g: carbsG,
    p_target_fat_g: fatG,
  });
}
...
// Defaults keep the old behaviour: USER_SUPPLIED, dated today.
// Receipt path passes source: 'RECEIPT' + fetchedAt: <receipt date>.
export async function setIngredientPrice({ ingredientId, pricePence, packSize = null, source = 'USER_SUPPLIED', fetchedAt = null }) {
  const supabase = getBrowserClient();
  return supabase.rpc('set_ingredient_price', {
    ...
    p_source: source,
    p_fetched_at: fetchedAt, // null → SQL coalesces to current_date
  });
}
